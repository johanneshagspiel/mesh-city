...
from object_detection.utils import label_map_util, metrics, per_image_evaluation
...
class ObjectDetectionEvaluator(DetectionEvaluator):
	...
	def _build_metric_names(self):
		...
		for idx in range(self._num_classes):
			if idx + self._label_id_offset in category_index:
				...
				self._metric_names.append(
					self._metric_prefix + 'PerformanceByCategory/AP@{}IOU/{}'.
    ...
    """
		...
		if (
			standard_fields.InputDataFields.groundtruth_difficult in six.viewkeys(groundtruth_dict) and
			(
			groundtruth_dict[standard_fields.InputDataFields.groundtruth_difficult].size or
			...
			if not len(self._image_ids) % 1000:
				logging.warning(
					'image %s does not have groundtruth difficult flag specified', image_id
				)
		...
		for idx in range(per_class_ap.size):
			if idx + self._label_id_offset in category_index:
				...
				if self._evaluate_precision_recall:
					display_name = (
						self._metric_prefix + 'PerformanceByCategory/Precision@{}IOU/{}'.
						format(self._matching_iou_threshold, category_name)
					)
					pascal_metrics[display_name] = per_class_precision[idx]
					display_name = (
						self._metric_prefix + 'PerformanceByCategory/Recall@{}IOU/{}'.
						format(self._matching_iou_threshold, category_name)
					)
...
class PrecisionAtRecallDetectionEvaluator(ObjectDetectionEvaluator):
	...
	def __init__(
		self, categories, matching_iou_threshold=0.5, recall_lower_bound=0.0, recall_upper_bound=1.0
	):
...
class OpenImagesDetectionEvaluator(ObjectDetectionEvaluator):
  ...
  """
    ...
    """
		...
		# use values from the dictionary or insert None otherwise.
		if (
			standard_fields.InputDataFields.groundtruth_group_of in six.viewkeys(groundtruth_dict) and
			(
			groundtruth_dict[standard_fields.InputDataFields.groundtruth_group_of].size or
			not groundtruth_classes.size
			)
		):
			...
			if not len(self._image_ids) % 1000:
				logging.warning(
					'image %s does not have groundtruth group_of flag specified', image_id
				)
		if self._evaluate_masks:
			groundtruth_masks = groundtruth_dict[
				standard_fields.InputDataFields.groundtruth_instance_masks]
...
class ObjectDetectionEvaluation(object):
    ...
    """
		if image_key in self.groundtruth_boxes:
			logging.warning(
				'image %s has already been added to the ground truth database.', image_key
			)
			return
		...
		self.groundtruth_is_difficult_list[image_key] = groundtruth_is_difficult_list.astype(
			dtype=bool
		)
		...
		self.groundtruth_is_group_of_list[image_key] = groundtruth_is_group_of_list.astype(
			dtype=bool
		)
    ...
    """
		if (
			len(detected_boxes) != len(detected_scores) or
		...
		if image_key in self.detection_keys:
			logging.warning(
				'image %s has already been added to the detection result database', image_key
			)
			return
