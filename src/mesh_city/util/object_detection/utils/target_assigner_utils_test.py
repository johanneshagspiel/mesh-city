...
from object_detection.utils import target_assigner_utils as ta_utils, test_case
...
class TargetUtilTest(test_case.TestCase):
	...
	def test_coordinates_to_heatmap(self):
		...
		def graph_fn():
			...
			heatmap = ta_utils.coordinates_to_heatmap(
				y_grid, x_grid, y_coordinates, x_coordinates, sigma, channel_onehot, channel_weights
			)
			return heatmap
	...
	def test_compute_floor_offsets_with_indices_onlysource(self):
		...
		def graph_fn():
			...
			(offsets, indices) = ta_utils.compute_floor_offsets_with_indices(y_source, x_source)

			return offsets, indices

		offsets, indices = self.execute(graph_fn, [])

		np.testing.assert_array_almost_equal(offsets, np.array([[0.5, 0.5], [0.3, 0.2]]))
	...
	def test_compute_floor_offsets_with_indices_and_targets(self):
		...
		def graph_fn():
			...
			(offsets, indices
			) = ta_utils.compute_floor_offsets_with_indices(y_source, x_source, y_target, x_target)
			return offsets, indices

		offsets, indices = self.execute(graph_fn, [])

		np.testing.assert_array_almost_equal(offsets, np.array([[1.1, -0.8], [0.1, 0.5]]))
		np.testing.assert_array_almost_equal(indices, np.array([[1, 2], [0, 4]]))
	...
	def test_get_valid_keypoints_mask(self):
		...
		def graph_fn():
			...
			keypoints = tf.constant(
				[[0.1, float('nan'), 0.2, 0.0], [0.0, 0.0, 0.1, 0.9], [3.2, 4.3, float('nan'), 0.2]],
				dtype=tf.float32
			)
	...
	def test_blackout_pixel_weights_by_box_regions(self):
		...
		def graph_fn():
			boxes = tf.constant(
				[[0.0, 0.0, 5, 5], [0.0, 0.0, 10.0, 20.0], [6.0, 12.0, 8.0, 18.0]], dtype=tf.float32
			)
