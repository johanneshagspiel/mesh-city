...
from object_detection.utils import test_case, visualization_utils
...
class VisualizationUtilsTest(test_case.TestCase):
	...
	def test_draw_bounding_boxes_on_image_tensors_with_track_ids(self):
		...
		def graph_fn():
			...
			boxes = tf.constant(
				[
				[[0.4, 0.25, 0.75, 0.75], [0.5, 0.3, 0.7, 0.9], [0.7, 0.5, 0.8, 0.9]],
				[[0.41, 0.25, 0.75, 0.75], [0.51, 0.3, 0.7, 0.9], [0.75, 0.5, 0.8, 0.9]]
				]
			)
	...
	def test_draw_bounding_boxes_on_image_tensors_with_additional_channels(self):
		...
		def graph_fn():
			...
			images_with_boxes = (
				visualization_utils.draw_bounding_boxes_on_image_tensors(
				images_tensor, boxes, classes, scores, category_index, min_score_thresh=0.2
				)
			)
	...
	def test_draw_mask_on_image_array(self):
		...
		visualization_utils.draw_mask_on_image_array(test_image, mask, color='Blue', alpha=.5)
	...
	def test_draw_heatmaps_on_image_array(self):
		...
		output_image = visualization_utils.draw_heatmaps_on_image_array(test_image, heatmaps)
		# Output image should be painted as "AliceBlue" at (0, 0), (1, 1)
		# and "Chartreuse" at (0, 1), (1, 0).
		self.assertAllEqual(
			output_image,
			np.array([[[240, 248, 255], [127, 255, 0]], [[127, 255, 0], [240, 248, 255]]])
		)
	...
	def test_draw_heatmaps_on_image_tensors(self):
		...
		def graph_fn():
			...
			heatmaps_tensor = tf.expand_dims(tf.constant(heatmaps, dtype=tf.float32), axis=0)
		...
		self.assertAllEqual(
			output_image_np,
			np.expand_dims(
			np.array([[[240, 248, 255], [127, 255, 0]], [[127, 255, 0], [240, 248, 255]]]), axis=0
			)
		)
	...
	def test_eval_metric_ops(self):
		...
		detection_scores = tf.random_uniform([4, 20], minval=0., maxval=1., dtype=tf.float32)
		...
		eval_dict = {
			...
			fields.InputDataFields.original_image_spatial_shape: (original_image_spatial_shape),
		...
		with self.test_session() as sess:
			...
			for i in range(max_examples_to_draw):
				...
				sess.run(
					...
					feed_dict={
					original_image:
					np.random.randint(low=0, high=256, size=(4, 6 + i, 7 + i, 3), dtype=np.uint8),
					original_image_spatial_shape:
					[[6 + i, 7 + i], [6 + i, 7 + i], [6 + i, 7 + i], [6 + i, 7 + i]],
					true_image_shape:
			...
			value_ops_out = sess.run(value_ops)
			self.assertEqual(
				six.b(''), value_ops_out[metric_op_base + '/' + str(max_examples_to_draw - 1)]
			)
	...
	def test_visualize_boxes_and_labels_on_image_array(self):
		...
		detections = np.array([[0.8, 0.1, 0.9, 0.1, 1., 0.1], [0.1, 0.3, 0.8, 0.7, 1., 0.6]])
