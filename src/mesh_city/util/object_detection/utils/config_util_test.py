...
from object_detection.protos import (
	eval_pb2, image_resizer_pb2, input_reader_pb2, model_pb2, pipeline_pb2, train_pb2,
)
...
class ConfigUtilTest(tf.test.TestCase):
	...
	def _assertOptimizerWithNewLearningRate(self, optimizer_name):
		...
		for i, schedule in enumerate(manual_lr.schedule):
			self.assertAlmostEqual(
				hparams.learning_rate * learning_rate_scaling**i, schedule.learning_rate
			)
	...
	def testNewFocalLossParameters(self):
		...
		new_alpha = 0.3
		new_gamma = 2.0
		hparams = contrib_training.HParams(focal_loss_alpha=new_alpha, focal_loss_gamma=new_gamma)
		pipeline_config_path = os.path.join(self.get_temp_dir(), "pipeline.config")
	...
	def testOverwriteAllEvalSampling(self):
		...
		).sample_1_of_n_examples = (original_num_eval_examples)
		pipeline_config.eval_input_reader.add(
		).sample_1_of_n_examples = (original_num_eval_examples)
		_write_config(pipeline_config, pipeline_config_path)
	...
	def testUpdateNumClasses(self):
		...
		config_util.merge_external_params_with_configs(configs, kwargs_dict={"num_classes": 2})
