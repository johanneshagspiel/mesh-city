...
from mesh_city.util.object_detection.core import standard_fields as fields
from mesh_city.util.object_detection.utils import (
	shape_utils, spatial_transform_ops as spatial_ops, static_shape,
)
...
def normalized_to_image_coordinates(normalized_boxes, image_shape, parallel_iterations=32):
  ...
  """
	...
	def _to_absolute_coordinates(normalized_boxes):
		y_min, x_min, y_max, x_max = tf.split(value=normalized_boxes, num_or_size_splits=4, axis=1)
		y_min = y_scale * y_min
		y_max = y_scale * y_max
		x_min = x_scale * x_min
		x_max = x_scale * x_max
  ...
  """
	...
	if tensor_height is None:
		...
		padded_tensor_height = tf.cast(
			tf.ceil(tf.cast(tensor_height, dtype=tf.float32) / tf.cast(multiple, dtype=tf.float32)),
			dtype=tf.int32
		) * multiple
	else:
	...
	if tensor_width is None:
		...
		padded_tensor_width = tf.cast(
			tf.ceil(tf.cast(tensor_width, dtype=tf.float32) / tf.cast(multiple, dtype=tf.float32)),
			dtype=tf.int32
		) * multiple
	else:
	...
	if padded_tensor_height != tensor_height:
		height_pad = tf.zeros(
			[batch_size, padded_tensor_height - tensor_height, tensor_width, tensor_depth]
		)
		tensor = tf.concat([tensor, height_pad], 1)
	if padded_tensor_width != tensor_width:
		width_pad = tf.zeros(
			[batch_size, padded_tensor_height, padded_tensor_width - tensor_width, tensor_depth]
		)
...
def padded_one_hot_encoding(indices, depth, left_pad):
  ...
  """
	...
	def one_hot_and_pad():
		one_hot = tf.cast(
			tf.one_hot(tf.cast(indices, tf.int64), depth, on_value=1, off_value=0), tf.float32
		)
		return tf.pad(one_hot, [[0, 0], [left_pad, 0]], mode='CONSTANT')

	result = tf.cond(
		tf.greater(tf.size(indices), 0), one_hot_and_pad, lambda: tf.zeros((depth + left_pad, 0))
	)
...
def retain_groundtruth(tensor_dict, valid_indices):
  ...
  """
	...
	if fields.InputDataFields.groundtruth_boxes in tensor_dict:
		...
		for key in tensor_dict:
			...
			# Needs to reshape into [num_boxes, -1] for tf.gather() to work.
				...
				valid_dict[key] = tf.reshape(
					tf.gather(tf.reshape(tensor_dict[key], [num_boxes, -1]), valid_indices), [-1]
				)
		...
		raise ValueError(
			'%s not present in input tensor dict.' % (fields.InputDataFields.groundtruth_boxes)
		)
	return valid_dict
...
def filter_groundtruth_with_nan_box_coordinates(tensor_dict):
  ...
  """
	...
	nan_indicator_vector = tf.greater(
		tf.reduce_sum(tf.cast(tf.is_nan(groundtruth_boxes), dtype=tf.int32), reduction_indices=[1]), 0
	)
...
def filter_unrecognized_classes(tensor_dict):
  ...
  """
	...
	recognized_indices = tf.where(
		tf.greater(tensor_dict[fields.InputDataFields.groundtruth_classes], unrecognized_label)
	)
...
def normalize_to_target(
  ...
  """
	with tf.variable_scope(scope, 'NormalizeToTarget', [inputs]):
		...
		if not (
			isinstance(target_norm_value, float) or
			(isinstance(target_norm_value, list) and len(target_norm_value) == depth) and
	...
	for (split, box) in zip(image_splits, position_sensitive_boxes):
		if split.shape.is_fully_defined() and box.shape.is_fully_defined():
			crop = tf.squeeze(
				...
				tf.expand_dims(split, axis=0), tf.expand_dims(box, axis=0), bin_crop_size
				),
				axis=0
			)
		else:
...
def merge_boxes_with_multiple_labels(
  ...
  """
	...
	box_confidence_shape_assert = (
		shape_utils.assert_shape_equal_along_first_dimension(boxes_shape, confidences_shape)
	)
	...
	with tf.control_dependencies(
		...
		hashcodes = (
			ymin + xmin * quantization_bins + ymax * quantization_bins * quantization_bins +
		...
		def map_box_encodings(i):
			...
			box_class_encodings = tf.sparse_to_dense(
				box_indices, [num_classes], tf.constant(1, dtype=tf.int64), validate_indices=False
			)
...
def nearest_neighbor_upsampling(input_tensor, scale=None, height_scale=None, width_scale=None):
  ...
  """
	...
	with tf.name_scope('nearest_neighbor_upsampling'):
		...
		return tf.reshape(output_tensor, [batch_size, height * h_scale, width * w_scale, channels])
...
def fpn_feature_levels(num_levels, unit_scale_index, image_ratio, boxes):
  ...
  """
	...
	assert unit_scale_index < num_levels and unit_scale_index >= 0, (
		'`unit_scale_index` must be in [0, {}). Found {}.'.format(num_levels, unit_scale_index)
	)
	...
	levels = tf.cast(
		tf.floordiv(tf.log(areas_sqrt * image_ratio), log_2) + unit_scale_index, dtype=tf.int32
	)
...
def bfloat16_to_float32_nested(input_nested):
  ...
  """
	if isinstance(input_nested, tf.Tensor):
		...
		out_tensor_dict = {k: bfloat16_to_float32_nested(v) for k, v in input_nested.items()}
	else:
		return input_nested
...
def tile_context_tensors(tensor_dict):
	...
	for key in tensor_dict:
		if key not in fields.SEQUENCE_FIELDS:
			...
			tensor_shape = shape_utils.combined_static_and_dynamic_shape(original_tensor)
...
def decode_image(tensor_dict):
	...
	tensor_dict[fields.InputDataFields.image
				] = tf.io.decode_image(tensor_dict[fields.InputDataFields.image], channels=3)
