...
class OpenImagesV2EvaluationTest(tf.test.TestCase):
	...
	def test_returns_correct_metric_values(self):
		...
		oiv2_evaluator.add_single_ground_truth_image_info(
			...
			standard_fields.InputDataFields.groundtruth_group_of: np.array([], dtype=bool)
			}
		)
		image_key2 = 'img2'
...
class OpenImagesChallengeEvaluatorTest(tf.test.TestCase):
	...
	def test_returns_correct_detection_metric_values(self):
		...
		image_key = 'img2'
		detected_boxes = np.array(
			[[10, 10, 11, 11], [100, 100, 120, 120], [100, 100, 220, 220], [10, 10, 11, 11]],
			dtype=float
		)
		...
		self.assertAlmostEqual(
			metrics[expected_metric_name + '_PerformanceByCategory/AP@0.5IOU/dog'], 0.3333333333
		)
		self.assertAlmostEqual(
		...
		self.assertAlmostEqual(
			metrics[expected_metric_name + '_PerformanceByCategory/AP@0.5IOU/cat'], 0.142857142857
		)
	...
	def test_returns_correct_instance_segm_metric_values(self):
		...
		oivchallenge_evaluator = (
			object_detection_evaluation.OpenImagesChallengeEvaluator(categories, evaluate_masks=True)
		)
...
class PascalEvaluationTest(tf.test.TestCase):
	...
	def test_returns_correct_metric_values_on_boxes(self):
		...
		pascal_evaluator.add_single_ground_truth_image_info(
			...
			standard_fields.InputDataFields.groundtruth_boxes: groundtruth_boxes1,
			standard_fields.InputDataFields.groundtruth_classes: groundtruth_class_labels1,
			standard_fields.InputDataFields.groundtruth_difficult: np.array([], dtype=bool)
			}
		)
		image_key2 = 'img2'
		groundtruth_boxes2 = np.array(
	...
	def test_returns_correct_metric_values_on_masks(self):
		...
		groundtruth_masks1 = np.stack(
			[groundtruth_masks_1_0, groundtruth_masks_1_1, groundtruth_masks_1_2], axis=0
		)
		...
		groundtruth_masks2 = np.stack(
			[groundtruth_masks_2_0, groundtruth_masks_2_1, groundtruth_masks_2_2], axis=0
		)
		pascal_evaluator.add_single_ground_truth_image_info(
			image_key2,
			{
...
class WeightedPascalEvaluationTest(tf.test.TestCase):
	...
	def create_and_add_common_ground_truth(self):
		#  Add groundtruth
		self.wp_eval = (
			object_detection_evaluation.WeightedPascalDetectionEvaluator(self.categories)
		)
	...
	def test_returns_correct_metric_values(self):
		...
		self.assertAlmostEqual(
			metrics[self.wp_eval._metric_prefix + 'PerformanceByCategory/AP@0.5IOU/dog'], 0.0
		)
		self.assertAlmostEqual(
			metrics[self.wp_eval._metric_prefix + 'PerformanceByCategory/AP@0.5IOU/elephant'], 0.0
		)
		self.assertAlmostEqual(
			metrics[self.wp_eval._metric_prefix + 'PerformanceByCategory/AP@0.5IOU/cat'], 0.5 / 4
		)
		self.assertAlmostEqual(
			metrics[self.wp_eval._metric_prefix + 'Precision/mAP@0.5IOU'], 1. / (4 + 1 + 2) / 3
		)
		self.wp_eval.clear()
	...
	def test_returns_correct_metric_values_with_difficult_list(self):
		...
		self.assertAlmostEqual(
			metrics[self.wp_eval._metric_prefix + 'PerformanceByCategory/AP@0.5IOU/cat'], 0.5 / 3
		)
		self.assertAlmostEqual(
			metrics[self.wp_eval._metric_prefix + 'Precision/mAP@0.5IOU'], 1. / (3 + 1 + 2) / 3
		)
		self.wp_eval.clear()
		self.assertFalse(self.wp_eval._image_ids)
...
class PrecisionAtRecallEvaluationTest(tf.test.TestCase):
	...
	def test_returns_correct_metric_values(self):
		...
		self.assertAlmostEqual(
			metrics[self.wp_eval._metric_prefix + 'Precision/mAP@0.5IOU@[0.0,0.5]Recall'],
			1. / (3 + 1 + 2) / 4
		)
		self.wp_eval.clear()
		self.assertFalse(self.wp_eval._image_ids)
...
class ObjectDetectionEvaluationTest(tf.test.TestCase):
	...
	def test_add_single_ground_truth_image_info(self):
		...
		self.assertTrue(
			np.array_equal(
			expected_num_gt_instances_per_class, self.od_eval.num_gt_instances_per_class
			)
		)
		self.assertTrue(
			np.array_equal(expected_num_gt_imgs_per_class, self.od_eval.num_gt_imgs_per_class)
		)
		groundtruth_boxes2 = np.array(
			[[10, 10, 11, 11], [500, 500, 510, 510], [10, 10, 12, 12]], dtype=float
		)
		...
		self.assertTrue(
			np.allclose(
			self.od_eval.groundtruth_is_difficult_list['img2'], groundtruth_is_difficult_list2
			)
		)
		groundtruth_is_group_of_list2 = np.array([False, False, True], dtype=bool)
		self.assertTrue(
			np.allclose(
			self.od_eval.groundtruth_is_group_of_list['img2'], groundtruth_is_group_of_list2
			)
		)

		groundtruth_class_labels1 = np.array([0, 2, 0], dtype=int)
		self.assertTrue(
			np.array_equal(self.od_eval.groundtruth_class_labels['img1'], groundtruth_class_labels1)
		)

	def test_add_single_detected_image_info(self):
		expected_scores_per_class = [
			[np.array([0.8, 0.7], dtype=float)], [], [np.array([0.9], dtype=float)]
		]
		expected_tp_fp_labels_per_class = [
			[np.array([0, 1], dtype=bool)], [], [np.array([0], dtype=bool)]
		]
		...
		for i in range(self.od_eval.num_class):
			for j in range(len(expected_scores_per_class[i])):
				self.assertTrue(
					np.allclose(expected_scores_per_class[i][j], self.od_eval.scores_per_class[i][j])
				)
				self.assertTrue(
					np.array_equal(
					expected_tp_fp_labels_per_class[i][j], self.od_eval.tp_fp_labels_per_class[i][j]
					)
				)
		self.assertTrue(
			np.array_equal(
	...
	def test_evaluate(self):
		...
		expected_precisions_per_class = [
			np.array([0, 0.5], dtype=float), np.array([], dtype=float), np.array([0], dtype=float)
		]
		expected_recalls_per_class = [
			np.array([0, 1. / 3.], dtype=float), np.array([], dtype=float), np.array([0], dtype=float)
		]
		...
		self.assertTrue(
			np.allclose(expected_average_precision_per_class, average_precision_per_class)
		)
...
class ObjectDetectionEvaluatorTest(tf.test.TestCase, parameterized.TestCase):
	...
	def _make_evaluation_dict(
		...
		detection_boxes = tf.concat(
			[
			tf.tile(tf.constant([[[0., 0., 1., 1.]]]), multiples=[batch_size - 1, 1, 1]),
			tf.constant([[[0., 0., 0.5, 0.5]]])
			],
			axis=0
		)
		detection_scores = tf.concat(
			[tf.tile(tf.constant([[0.5]]), multiples=[batch_size - 1, 1]), tf.constant([[0.8]])],
			axis=0
		)
		detection_classes = tf.tile(tf.constant([[0]]), multiples=[batch_size, 1])
		detection_masks = tf.tile(
			tf.ones(shape=[1, 2, 20, 20], dtype=tf.float32), multiples=[batch_size, 1, 1, 1]
		)
		...
		if batch_size > 1:
			...
			groundtruth_instance_masks = tf.tile(
				tf.expand_dims(groundtruth_instance_masks, 0), multiples=[batch_size, 1, 1, 1]
			)
	...
	@parameterized.parameters(
		{
		'batch_size': 1, 'expected_map': 0, 'max_gt_boxes': None, 'scale_to_absolute': True
		}, {
		'batch_size': 8, 'expected_map': 0.765625, 'max_gt_boxes': [1], 'scale_to_absolute': True
		}, {
		'batch_size': 1, 'expected_map': 0, 'max_gt_boxes': None, 'scale_to_absolute': False
		}, {
		'batch_size': 8, 'expected_map': 0.765625, 'max_gt_boxes': [1], 'scale_to_absolute': False
		}
	)
	def test_get_estimator_eval_metric_ops(
		...
		eval_dict = self._make_evaluation_dict(
			batch_size=batch_size, max_gt_boxes=max_gt_boxes, scale_to_absolute=scale_to_absolute
		)
