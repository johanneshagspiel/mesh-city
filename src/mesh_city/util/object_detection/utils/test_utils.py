...
from object_detection.core import anchor_generator, box_coder, box_list, box_predictor, matcher
from object_detection.utils import shape_utils, tf_version
...
class MockMaskHead(object):
	...
	def predict(self, features):
		...
		return tf.zeros(
			(batch_size, 1, self._num_classes, DEFAULT_MASK_SIZE, DEFAULT_MASK_SIZE), dtype=tf.float32
		)
...
class MockBoxPredictor(box_predictor.BoxPredictor):
	...
	def _predict(self, image_features, num_predictions_per_location):
		...
		combined_feature_shape = shape_utils.combined_static_and_dynamic_shape(image_feature)
...
class MockKerasBoxPredictor(box_predictor.KerasBoxPredictor):
	...
	def __init__(self, is_training, num_classes, add_background_class=True):
		super(MockKerasBoxPredictor, self).__init__(is_training, num_classes, False, False)
		self._add_background_class = add_background_class
...
def first_rows_close_as_set(a, b, k=None, rtol=1e-6, atol=1e-6):
  ...
  """
	...
	b_sorted = sorted(b[:k])
	return all(
		[np.allclose(entry_a, entry_b, rtol, atol) for (entry_a, entry_b) in zip(a_sorted, b_sorted)]
	)
