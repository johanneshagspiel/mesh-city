...
from mesh_city.util.object_detection.core import keypoint_ops, standard_fields as fields
from mesh_city.util.object_detection.utils import shape_utils
...
def draw_bounding_box_on_image(
  ...
  """
	...
	if use_normalized_coordinates:
		(left, right, top,
			bottom) = (xmin * im_width, xmax * im_width, ymin * im_height, ymax * im_height)
	else:
  ...
  """
	...
	if len(eval_dict[detection_fields.detection_classes].shape) == 1:
		for key in eval_dict:
			if (
				key != input_data_fields.original_image and
	...
	for indx in range(eval_dict[input_data_fields.original_image].shape[0]):
		...
		if detection_fields.detection_keypoints in eval_dict:
			...
			if detection_fields.detection_keypoint_scores in eval_dict:
				...
				keypoint_scores = tf.cast(
					keypoint_ops.set_keypoint_visibilities(keypoints), dtype=tf.float32
				)

		groundtruth_instance_masks = None
		if input_data_fields.groundtruth_instance_masks in eval_dict:
			groundtruth_instance_masks = tf.cast(
				tf.expand_dims(eval_dict[input_data_fields.groundtruth_instance_masks][indx], axis=0),
				tf.uint8
			)
		...
		if input_data_fields.groundtruth_keypoints in eval_dict:
			...
			if gt_kpt_vis_fld in eval_dict:
				...
				groundtruth_keypoint_scores = tf.cast(
					keypoint_ops.set_keypoint_visibilities(groundtruth_keypoints), dtype=tf.float32
				)
		...
		images_with_groundtruth = draw_bounding_boxes_on_image_tensors(
			...
			tf.expand_dims(
			tf.ones_like(eval_dict[input_data_fields.groundtruth_classes][indx], dtype=tf.float32),
			axis=0
			),
			category_index,
			original_image_spatial_shape=tf.expand_dims(
		...
		if input_data_fields.image_additional_channels in eval_dict:
			...
			images_to_visualize = tf.concat(
				[images_to_visualize, images_with_additional_channels_groundtruth], axis=2
			)
  ...
  """
	...
	if keypoint_edges is not None:
		for keypoint_start, keypoint_end in keypoint_edges:
			if (
				keypoint_start < 0 or keypoint_start >= len(keypoints) or keypoint_end < 0 or
				keypoint_end >= len(keypoints)
			):
				continue
