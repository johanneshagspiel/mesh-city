  ...
  """
	...
	if apply_batchnorm:
		layers.append(
			...
			training=(is_training and not freeze_batchnorm), name=name + '_batchnorm'
			)
		)

	if apply_activation:
		layers.append(conv_hyperparams.build_activation_layer(name=name + '_activation'))
...
def create_downsample_feature_map_ops(
  ...
  """
	...
	if downsample_method == 'max_pooling':
		...
		layers.append(
			conv_hyperparams.build_batch_norm(
			training=(is_training and not freeze_batchnorm), name=name + '_downsample_batchnorm'
			)
		)
		layers.append(conv_hyperparams.build_activation_layer(name=name + '_downsample_activation'))
	else:
  ...
  """
	...
	if use_native_resize_op:
		...
		layers.append(
			...
			resize_nearest_neighbor, name=name + 'nearest_neighbor_upsampling_x{}'.format(scale)
			)
		)
	else:
		...
		layers.append(
			tf.keras.layers.Lambda(
			nearest_neighbor_upsampling, name=name + 'nearest_neighbor_upsampling_x{}'.format(scale)
			)
		)
...
class BiFPNCombineLayer(tf.keras.layers.Layer):
  ...
  """
	...
	def _combine_weighted_sum(self, inputs):
		return tf.squeeze(
			tf.linalg.matmul(tf.stack(inputs, axis=-1), self.per_input_weights), axis=[-1]
		)
	...
	def build(self, input_shape):
		if not isinstance(input_shape, list):
			raise ValueError('A BiFPN combine layer should be called ' 'on a list of inputs.')
