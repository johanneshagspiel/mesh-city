...
class PerImageEvaluation(object):
    ...
    """
		if (detected_masks is not None and
			groundtruth_masks is None) or (detected_masks is None and groundtruth_masks is not None):
			raise ValueError(
    ...
    """
		...
		if mask_mode:
			...
			if sum(mask_presence_indicator) < len(mask_presence_indicator):
				...
				(iou, ioa, _, num_detected_boxes) = self._get_overlaps_and_scores_box_mode(
					detected_boxes=detected_boxes,
					detected_scores=detected_scores,
					groundtruth_boxes=groundtruth_boxes[~mask_presence_indicator, :],
					groundtruth_is_group_of_list=groundtruth_is_group_of_list[~mask_presence_indicator]
      ...
      """
			...
			for i in range(num_detected_boxes):
				...
				is_evaluatable = (
					not tp_fp_labels[i] and not is_matched_to_difficult[i] and
					iou[i, gt_id] >= self.matching_iou_threshold and not is_matched_to_group_of[i]
				)
				if is_evaluatable:
      ...
      """
			...
			for i in range(num_detected_boxes):
				...
				is_evaluatable = (
					not tp_fp_labels[i] and not is_matched_to_difficult[i] and
					ioa[i, gt_id] >= self.matching_iou_threshold and not is_matched_to_group_of[i]
				)
				if is_evaluatable:
		...
		if iou.shape[1] > 0:
			...
			compute_match_iou(
				iou, groundtruth_is_difficult_box_list[~groundtruth_is_group_of_box_list], is_box=True
			)
