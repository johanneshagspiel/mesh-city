...
class SingleClassTpFpWithGroupOfBoxesTestWeighted(tf.test.TestCase):
	...
	def test_match_two_to_group_of_box(self):
		...
		tf.logging.info(
			"test_match_two_to_group_of_box {} {}".format(tp_fp_labels, expected_tp_fp_labels)
		)
	...
	def test_mask_match_two_to_group_of_box(self):
		...
		tf.logging.info(
			"test_mask_match_two_to_group_of_box {} {}".format(tp_fp_labels, expected_tp_fp_labels)
		)
...
class MultiClassesTpFpTest(tf.test.TestCase):
	...
	def test_tp_fp(self):
		...
		detected_boxes = np.array(
			[[0, 0, 1, 1], [10, 10, 5, 5], [0, 0, 2, 2], [5, 10, 10, 5], [10, 5, 5, 10], [0, 0, 3, 3]],
			dtype=float
		)
