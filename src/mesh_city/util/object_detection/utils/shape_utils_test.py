...
from object_detection.utils import shape_utils, test_case
...
class UtilTest(test_case.TestCase):
	...
	def test_pad_or_clip_nd_tensor(self):
		...
		def graph_fn(input_tensor):
			output_tensor = shape_utils.pad_or_clip_nd(input_tensor, [None, 3, 5, tf.constant(6)])
...
class StaticOrDynamicMapFnTest(test_case.TestCase):
	...
	def test_with_dynamic_shape(self):
		...
		result1 = self.execute(graph_fn, [np.array([[1, 2], [3, 1], [0, 4]]), ])
		result2 = self.execute(graph_fn, [np.array([[-1, 1], [0, 9]]), ])
	...
	def test_with_multiple_dynamic_shapes(self):
		...
		result1 = self.execute(
			graph_fn, [np.array([[1, 2, 3], [4, 5, -1], [0, 6, 9]]), np.array([[0], [2], [1]]), ]
		)
		result2 = self.execute(graph_fn, [np.array([[-1, 1, 0], [3, 9, 30]]), np.array([[1], [0]])])
...
class AssertShapeEqualTest(test_case.TestCase):
	...
	def test_unequal_dynamic_shape_raises_tf_assert(self):
		...
		self.assertRaises(
			ValueError, self.execute, graph_fn, [np.zeros([1, 2, 2, 3]), np.zeros([1, 4, 4, 3])]
		)
	...
	def test_unequal_dynamic_shape_along_first_dim_raises_tf_assert(self):
		...
		self.assertRaises(
			ValueError, self.execute, graph_fn, [np.zeros([1, 2, 2, 3]), np.zeros([2, 4, 3])]
		)
...
class FlattenExpandDimensionTest(test_case.TestCase):
	...
	def test_raises_value_error_incorrect_dimensions(self):
		inputs = tf.random_uniform([5, 2, 10, 10, 3])
		self.assertRaises(ValueError, shape_utils.flatten_dimensions, inputs, first=0, last=6)
