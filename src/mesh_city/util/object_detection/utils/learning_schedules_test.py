	...
	def testCosineDecayWithWarmup(self):
		...
		def graph_fn(global_step):
			...
			learning_rate = learning_schedules.cosine_decay_with_warmup(
				global_step, learning_rate_base, total_steps, warmup_learning_rate, warmup_steps
			)
			assert learning_rate.op.name.endswith('learning_rate')
			return (learning_rate, )
		...
		output_rates = [
			self.execute(graph_fn, [np.array(step).astype(np.int64)]) for step in input_global_steps
		]
