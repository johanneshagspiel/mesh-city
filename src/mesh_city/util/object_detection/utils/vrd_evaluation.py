...
from object_detection.utils import metrics, object_detection_evaluation, per_image_vrd_evaluation
  ...
  """
    ...
    """
		...
		verified_labels = groundtruth_dict.get(
			standard_fields.InputDataFields.groundtruth_image_classes, np.array([], dtype=int)
		)
    ...
    """
		...
		num_detections = detections_dict[standard_fields.DetectionResultFields.detection_boxes
										].shape[0]
    ...
    """
		...
		vrd_metrics = {
			(self._metric_prefix + 'weightedAP@{}IOU'.format(self._matching_iou_threshold)):
			weighted_average_precision,
			self._metric_prefix + 'mAP@{}IOU'.format(self._matching_iou_threshold):
			mean_average_precision,
			self._metric_prefix + 'Recall@50@{}IOU'.format(self._matching_iou_threshold):
			recall_50,
			self._metric_prefix + 'Recall@100@{}IOU'.format(self._matching_iou_threshold):
			recall_100,
		}
		if relationships:
			for key, average_precision in six.iteritems(average_precisions):
				vrd_metrics[self._metric_prefix +
					'AP@{}IOU/{}'.format(self._matching_iou_threshold, relationships[key])
							] = average_precision
		else:
			for key, average_precision in six.iteritems(average_precisions):
				vrd_metrics[self._metric_prefix +
					'AP@{}IOU/{}'.format(self._matching_iou_threshold, key)] = average_precision
  ...
  """
	...
	def __init__(self, matching_iou_threshold=0.5):
		super(VRDRelationDetectionEvaluator, self).__init__(
			matching_iou_threshold=matching_iou_threshold, metric_prefix='VRDMetric_Relationships'
		)
  ...
  """
	...
	def __init__(self, matching_iou_threshold=0.5):
		super(VRDPhraseDetectionEvaluator, self).__init__(
			matching_iou_threshold=matching_iou_threshold, metric_prefix='VRDMetric_Phrases'
		)
  ...
  """
    ...
    """
		...
		for relation_field_value, _ in (six.iteritems(self._num_gt_instances_per_relationship)):
			...
			self._average_precisions[relation_field_value] = metrics.compute_average_precision(
				precisions, recalls
			)
		...
		self._recall_50 = (
			metrics.compute_recall_at_k(self._tp_fp_labels, self._num_gt_instances, 50)
		)
		self._median_rank_50 = (metrics.compute_median_rank_at_k(self._tp_fp_labels, 50))
		self._recall_100 = (
			metrics.compute_recall_at_k(self._tp_fp_labels, self._num_gt_instances, 100)
		)
