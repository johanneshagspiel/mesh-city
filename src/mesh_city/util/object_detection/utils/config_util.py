...
from object_detection.protos import (
	eval_pb2, graph_rewriter_pb2, input_reader_pb2, model_pb2, pipeline_pb2, train_pb2,
)
  ...
  """
	...
	if image_resizer_config.HasField("keep_aspect_ratio_resizer"):
		if image_resizer_config.keep_aspect_ratio_resizer.pad_to_max_dimension:
			...
			return [-1, -1]
	if image_resizer_config.HasField("identity_resizer") or image_resizer_config.HasField(
...
def _is_generic_key(key):
	...
	for prefix in [
		"graph_rewriter_config", "model", "train_input_config", "train_config", "eval_config"
	]:
...
def check_and_parse_input_config_key(configs, key):
  ...
  """
	...
	if field_name not in [
		"input_path", "label_map_path", "shuffle", "mask_type", "sample_1_of_n_examples"
	]:
  ...
  """
	...
	if is_valid_input_config_key:
		update_input_reader_config(
			configs, key_name=key_name, input_name=input_name, field_name=field_name, value=value
		)
  ...
  """
	if isinstance(configs[key_name], input_reader_pb2.InputReader):
		...
		raise ValueError(
			...
			"key_name:{}, input_name:{}, field_name:{}.".format(key_name, input_name, field_name)
		)
