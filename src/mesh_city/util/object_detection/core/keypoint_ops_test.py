...
class KeypointOpsTest(test_case.TestCase):
	...
	def test_scale(self):
		...
		def graph_fn():
			...
			expected_keypoints = tf.constant([[[0., 0.], [1.0, 1.0]], [[0.5, 0.6], [1.0, 0.7]]])
	...
	def test_keypoints_to_enclosing_bounding_boxes(self):
		...
		expected_bboxes = np.array([[3., 4., 8., 20.], [0., 3., 5., 19.]])
		self.assertAllClose(expected_bboxes, output)

	def test_to_normalized_coordinates(self):

		def graph_fn():
			keypoints = tf.constant([[[10., 30.], [30., 45.]], [[20., 0.], [40., 60.]]])
	...
	def test_to_absolute_coordinates(self):
		...
		def graph_fn():
			...
			expected_keypoints = tf.constant([[[10., 30.], [30., 45.]], [[20., 0.], [40., 60.]]])
			return output, expected_keypoints
	...
	def test_flip_horizontal(self):
		...
		def graph_fn():
			...
			expected_keypoints = tf.constant(
				[[[0.1, 0.9], [0.3, 0.7], [0.2, 0.8]], [[0.4, 0.6], [0.6, 0.4], [0.5, 0.5]], ]
			)
	...
	def test_flip_vertical(self):
		...
		def graph_fn():
			...
			expected_keypoints = tf.constant(
				[[[0.9, 0.1], [0.7, 0.3], [0.8, 0.2]], [[0.6, 0.4], [0.4, 0.6], [0.5, 0.5]], ]
			)
	...
	def test_rot90(self):
		...
		def graph_fn():
			...
			expected_keypoints = tf.constant(
				[[[0.9, 0.1], [0.8, 0.2], [0.7, 0.3]], [[0.4, 0.4], [0.4, 0.5], [0.3, 0.6]], ]
			)
	...
	def test_keypoint_weights_from_visibilities(self):
		...
		def graph_fn():
			keypoint_visibilities = tf.constant([[True, True, False], [False, True, False]])
	...
	def test_set_keypoint_visibilities_no_initial_kpt_vis(self):
		keypoints_np = np.array(
			[[[np.nan, 0.2], [np.nan, np.nan], [-3., 7.]], [[0.5, 0.2], [4., 1.0], [-3., np.nan]], ],
			dtype=np.float32
		)
	...
	def test_set_keypoint_visibilities(self):
		...
		initial_keypoint_visibilities_np = np.array(
			[
			[
			False, True,  # Will be overriden by NaN coords.
			False
			],  # Will be maintained, even though non-NaN coords.
			[
			True, False,  # Will be maintained, even though non-NaN coords.
			False
			]
			]
		)
