...
class BalancedPositiveNegativeSampler(minibatch_sampler.MinibatchSampler):
    ...
    """
		...
		one_hot_selector = tf.one_hot(
			tf.cast(indexed_positions, tf.int32) - 1, total_num_samples, dtype=tf.float32
		)
		return tf.cast(
			tf.tensordot(tf.cast(input_tensor, tf.float32), one_hot_selector, axes=[0, 0]), tf.int32
		)
    ...
    """
		...
		additional_false_sample = tf.less_equal(
			tf.cumsum(tf.cast(tf.logical_not(indicator), tf.float32)), batch_size - num_true_sampled
		)
		...
		sampled_idx = tf.multiply(
			tf.cast(tf.greater_equal(sampled_idx, tf.constant(0)), tf.int32), sampled_idx
		)
		...
		idx_indicator = tf.scatter_nd(
			tf.expand_dims(permutation, -1), sampled_idx_indicator, shape=(input_length, )
		)
		...
		with tf.name_scope(scope, 'BalancedPositiveNegativeSampler'):
			if self._is_static:
				...
				if batch_size is None:
					...
					max_num_neg = tf.cast(
						negative_positive_ratio * tf.cast(num_sampled_pos, dtype=tf.float32),
						dtype=tf.int32
					)
				else:
