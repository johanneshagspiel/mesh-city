...
class WeightedSigmoidClassificationLoss(Loss):
    ...
    """
		if class_indices is not None:
			weights *= tf.reshape(
				ops.indices_to_dense_vector(class_indices, tf.shape(prediction_tensor)[2]), [1, 1, -1]
			)
		per_entry_cross_ent = (
...
class SigmoidFocalClassificationLoss(Loss):
  ...
  """
    ...
    """
		...
		p_t = (
			(target_tensor * prediction_probabilities) +
		...
		if self._alpha is not None:
			alpha_weight_factor = (
				target_tensor * self._alpha + (1 - target_tensor) * (1 - self._alpha)
			)
  ...
  """
    ...
    """
		if self._bootstrap_type == 'soft':
			bootstrap_target_tensor = self._alpha * target_tensor + (1.0 -
				self._alpha) * tf.sigmoid(prediction_tensor)
		else:
			bootstrap_target_tensor = self._alpha * target_tensor + (1.0 -
				self._alpha) * tf.cast(tf.sigmoid(prediction_tensor) > 0.5, tf.float32)
		per_entry_cross_ent = (
  ...
  """
    ...
    """
		...
		max_negatives = tf.maximum(
			...
			tf.cast(
			max_negatives_per_positive * tf.cast(num_positives, dtype=tf.float32), dtype=tf.int32
			)
		)
