...
class MatchTest(test_case.TestCase):
	...
	def test_get_correct_counts(self):
		...
		def graph_fn():
			...
			return [
				num_matched_columns, num_unmatched_columns, num_ignored_columns, num_matched_rows
			]
	...
	def test_all_columns_accounted_for(self):
		...
		def graph_fn():
			...
			unmatched_column_indices = match.unmatched_column_indices()
			ignored_column_indices = match.ignored_column_indices()
			return (matched_column_indices, unmatched_column_indices, ignored_column_indices)
	...
	def test_scalar_gather_based_on_match(self):
		...
		def graph_fn():
			...
			gathered_tensor = match.gather_based_on_match(
				input_tensor, unmatched_value=100., ignored_value=200.
			)
