...
class BalancedPositiveNegativeSamplerTest(test_case.TestCase):
	...
	def test_subsample_all_examples(self):
		...
		self.assertEqual(sum(np.logical_and(np.logical_not(numpy_labels), is_sampled)), 32)
	...
	def test_subsample_all_examples_static(self):
		...
		def graph_fn(indicator, labels):
			sampler = (
				balanced_positive_negative_sampler.BalancedPositiveNegativeSampler(is_static=True)
			)
			return sampler.subsample(indicator, 64, labels)
	...
	def test_subsample_selection(self):
		...
		self.assertEqual(sum(np.logical_and(np.logical_not(numpy_labels), is_sampled)), 54)
	...
	def test_subsample_selection_larger_batch_size(self):
		...
		self.assertGreaterEqual(sum(np.logical_and(np.logical_not(numpy_labels), is_sampled)), 50)
	...
	def test_subsample_selection_no_batch_size(self):
		...
		self.assertEqual(sum(np.logical_and(np.logical_not(numpy_labels), is_sampled_out)), 495)
		self.assertAllEqual(is_sampled_out, np.logical_and(is_sampled_out, numpy_indicator))
