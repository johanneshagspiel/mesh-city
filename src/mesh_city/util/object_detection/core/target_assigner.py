...
from object_detection.box_coders import faster_rcnn_box_coder, mean_stddev_box_coder
...
from object_detection.utils import shape_utils, target_assigner_utils as ta_utils
...
def create_target_assigner(
  ...
  """
	...
	return TargetAssigner(
		similarity_calc, matcher, box_coder_instance, negative_class_weight=negative_class_weight
	)
...
def batch_get_targets(
	batch_match, groundtruth_tensor_list, groundtruth_weights_list, unmatched_value, unmatched_weight
):
  ...
  """
	...
	for match_tensor, groundtruth_tensor, groundtruth_weight in zip(
		...
		targets = match_object.gather_based_on_match(
			groundtruth_tensor, unmatched_value=unmatched_value, ignored_value=unmatched_value
		)
...
def batch_assign_confidences(
  ...
  """
	...
	for anchors, gt_boxes, gt_class_confidences, gt_weights in zip(
		...
		if (
			gt_class_confidences is not None and len(gt_class_confidences.get_shape().as_list()) > 2
		):
			raise ValueError(
				'The shape of the class target is not supported. ', gt_class_confidences.get_shape()
			)
		...
		if include_background_class:
			cls_weights_background = (
				(1 - implicit_class_weight) * regression_weights_expanded + implicit_class_weight
			)
