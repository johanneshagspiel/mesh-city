...
def batch_iou(boxes1, boxes2):
  ...
  """
	with tf.name_scope('BatchIOU'):
		...
		intersection_area = tf.maximum((intersection_xmax - intersection_xmin),
			0) * tf.maximum((intersection_ymax - intersection_ymin), 0)
...
def _self_suppression(iou, iou_threshold, loop_condition, iou_sum):
  ...
  """
	...
	iou_suppressed = tf.reshape(
		tf.cast(tf.reduce_max(can_suppress_others * iou, 1) <= iou_threshold, iou.dtype), [1, -1, 1]
	) * iou
...
def _suppression_loop_body(boxes, iou_threshold, output_size, idx):
  ...
  """
	...
	mask = tf.expand_dims(
		tf.reshape(tf.range(_NMS_TILE_SIZE), [1, -1]) > tf.reshape(tf.range(_NMS_TILE_SIZE), [-1, 1]),
		0
	)
	...
	boxes = tf.tile(tf.expand_dims(box_slice, [1]), [1, num_tiles, 1,
		1]) * mask + tf.reshape(boxes, [1, num_tiles, _NMS_TILE_SIZE, 4]) * (1 - mask)
...
def partitioned_non_max_suppression_padded(
  ...
  """
	...
	pad = tf.cast(
		tf.ceil(tf.cast(num_boxes, tf.float32) / _NMS_TILE_SIZE), tf.int32
	...
	def _loop_cond(unused_boxes, unused_threshold, output_size, idx):
		return tf.logical_and(
			tf.reduce_min(output_size) < max_output_size, idx < num_boxes // _NMS_TILE_SIZE
		)
...
def _validate_boxes_scores_iou_thresh(
  ...
  """
	...
	if not (
		shape_utils.get_dim_as_int(boxes.shape[1]) == shape_utils.get_dim_as_int(scores.shape[1]) or
	...
	with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
		...
		for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
			...
			if pad_to_max_output_size:
				...
				if (
					hasattr(tf.image, 'non_max_suppression_with_scores') and
			...
			nms_result.add_field(
				fields.BoxListFields.scores,
				tf.where(valid_nms_boxes_indices, selected_scores, -1 * tf.ones(max_selection_size))
			)
	...
	with tf.name_scope(scope, 'ClassAgnosticNonMaxSuppression'):
		...
		if pad_to_max_output_size:
			...
			if (
				...
				num_valid_nms_boxes = tf.shape(selected_indices)[0]
				selected_indices = tf.concat(
					[selected_indices, tf.zeros(max_selection_size - num_valid_nms_boxes, tf.int32)],
					0
				)
				selected_scores = tf.concat(
					[selected_scores, tf.zeros(max_selection_size - num_valid_nms_boxes, tf.float32)],
					-1
				)
				nms_result = box_list_ops.gather(boxlist_and_class_scores, selected_indices)
			else:
		...
		if max_total_size:
			...
			num_valid_nms_boxes = tf.where(
				max_total_size > num_valid_nms_boxes, num_valid_nms_boxes, max_total_size
			)
		# Select only the valid boxes if pad_to_max_output_size is False.
		if not pad_to_max_output_size:
...
def batch_multiclass_non_max_suppression(
  ...
  """
	if use_combined_nms:
		...
		if max_classes_per_detection > 1:
			tf.logging.warning('max_classes_per_detection is not configurable by combined_nms.')
      ...
      """
			...
			if use_static_shapes:
				...
				per_image_boxes = tf.reshape(
					tf.slice(per_image_boxes, 3 * [0], tf.stack([per_image_num_valid_boxes, -1, -1])),
					[-1, q, 4]
				)
				per_image_scores = tf.reshape(
					tf.slice(per_image_scores, [0, 0], tf.stack([per_image_num_valid_boxes, -1])),
					[-1, num_classes]
				)
				per_image_masks = tf.reshape(
					tf.slice(
					per_image_masks, 4 * [0], tf.stack([per_image_num_valid_boxes, -1, -1, -1])
					),
					[
					-1,
					q,
				...
				if per_image_additional_fields is not None:
					for key, tensor in per_image_additional_fields.items():
						...
						per_image_additional_fields[key] = tf.reshape(
							...
							tf.stack([per_image_num_valid_boxes] + (additional_field_dim - 1) * [-1])
							), [-1] +
							[shape_utils.get_dim_as_int(dim) for dim in additional_field_shape[1:]]
						)
			if use_class_agnostic_nms:
			...
			return (
				[nmsed_boxes, nmsed_scores, nmsed_classes, nmsed_masks] + nmsed_additional_fields +
				[num_detections]
			)
		...
		batch_outputs = map_fn(
			...
			elems=(
			[boxes, scores, masks, clip_window] + list(ordered_additional_fields.values()) +
			[num_valid_boxes]
			),
