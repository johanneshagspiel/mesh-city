...
def change_coordinate_frame(boxlist, window, scope=None):
  ...
  """
	with tf.name_scope(scope, 'ChangeCoordinateFrame'):
		...
		boxlist_new = scale(
			box_list.BoxList(boxlist.get() - [window[0], window[1], window[0], window[1]]),
			1.0 / win_height,
			1.0 / win_width
		)
...
def boolean_mask(
	boxlist, indicator, fields=None, scope=None, use_static_shapes=False, indicator_sum=None
):
  ...
  """
	with tf.name_scope(scope, 'VisualizeBoxesInImage'):
		if not normalized:
			...
			boxlist = scale(
				boxlist, 1.0 / tf.cast(height, tf.float32), 1.0 / tf.cast(width, tf.float32)
			)
  ...
  """
	with tf.name_scope(scope, 'NonMaxSuppression'):
		...
		selected_indices = tf.image.non_max_suppression(
			boxlist.get(), boxlist.get_field('scores'), max_output_size, iou_threshold=thresh
		)
...
def to_absolute_coordinates(
	boxlist, height, width, check_range=True, maximum_normalized_coordinate=1.1, scope=None
):
