...
from object_detection.core import (
	box_list, box_list_ops, keypoint_ops, preprocessor_cache, standard_fields as fields,
)
from object_detection.utils import autoaugment_utils, ops, patch_ops, shape_utils
...
def _apply_with_random_selector(x, func, num_cases, preprocess_vars_cache=None, key=''):
  ...
  """
	generator_func = functools.partial(tf.random_uniform, [], maxval=num_cases, dtype=tf.int32)
	rand_sel = _get_or_create_preprocess_rand_vars(
		generator_func, preprocessor_cache.PreprocessorCache.SELECTOR, preprocess_vars_cache, key
	)
...
def retain_boxes_above_threshold(
  ...
  """
	with tf.name_scope('RetainBoxesAboveThreshold', values=[boxes, labels, label_weights]):
...
def random_vertical_flip(
  ...
  """
	...
	with tf.name_scope('RandomVerticalFlip', values=[image, boxes]):
		...
		generator_func = functools.partial(tf.random_uniform, [], seed=seed)
		do_a_flip_random = _get_or_create_preprocess_rand_vars(
			generator_func, preprocessor_cache.PreprocessorCache.VERTICAL_FLIP, preprocess_vars_cache
		)
		do_a_flip_random = tf.greater(do_a_flip_random, 0.5)
  ...
  """
	...
	with tf.name_scope('RandomRotation90', values=[image, boxes]):
		...
		do_a_rot90_random = _get_or_create_preprocess_rand_vars(
			generator_func, preprocessor_cache.PreprocessorCache.ROTATION90, preprocess_vars_cache
		)
		...
		if keypoints is not None:
			keypoints = tf.cond(
				do_a_rot90_random, lambda: keypoint_ops.rot90(keypoints), lambda: keypoints
			)
			result.append(keypoints)
  ...
  """
	with tf.name_scope('RandomImageScale', values=[image]):
		...
		size_coef = _get_or_create_preprocess_rand_vars(
			generator_func, preprocessor_cache.PreprocessorCache.IMAGE_SCALE, preprocess_vars_cache
		)
  ...
  """
	...
	with tf.name_scope('RandomRGBtoGray', values=[image]):
		...
		do_gray_random = _get_or_create_preprocess_rand_vars(
			generator_func, preprocessor_cache.PreprocessorCache.RGB_TO_GRAY, preprocess_vars_cache
		)
...
def random_adjust_brightness(image, max_delta=0.2, seed=None, preprocess_vars_cache=None):
  ...
  """
	with tf.name_scope('RandomAdjustBrightness', values=[image]):
		generator_func = functools.partial(tf.random_uniform, [], -max_delta, max_delta, seed=seed)
...
def random_adjust_hue(image, max_delta=0.02, seed=None, preprocess_vars_cache=None):
  ...
  """
	with tf.name_scope('RandomAdjustHue', values=[image]):
		generator_func = functools.partial(tf.random_uniform, [], -max_delta, max_delta, seed=seed)
		delta = _get_or_create_preprocess_rand_vars(
			generator_func, preprocessor_cache.PreprocessorCache.ADJUST_HUE, preprocess_vars_cache
		)
	...
	with tf.name_scope('RandomDistortColor', values=[image]):
		if color_ordering == 0:
			image = random_adjust_brightness(
				image, max_delta=32. / 255., preprocess_vars_cache=preprocess_vars_cache
			)
			image = random_adjust_saturation(
				image, min_delta=0.5, max_delta=1.5, preprocess_vars_cache=preprocess_vars_cache
			)
...
def _strict_random_crop_image(
  ...
  """
	with tf.name_scope('RandomCropImage', values=[image, boxes]):
		...
		new_image = image[im_box_begin[0]:im_box_end[0], im_box_begin[1]:im_box_end[1], :]
		...
		if keypoint_visibilities is not None:
			kpt_vis_of_boxes_inside_window = tf.gather(keypoint_visibilities, inside_window_ids)
  ...
  """
	...
	if random_coef < sys.float_info.min:
		...
		do_a_crop_random = _get_or_create_preprocess_rand_vars(
			generator_func, preprocessor_cache.PreprocessorCache.CROP_IMAGE, preprocess_vars_cache
		)
...
def random_crop_pad_image(
  ...
  """
	...
	min_image_size = tf.cast(
		tf.cast(tf.stack([image_height, image_width]), dtype=tf.float32) * min_padded_size_ratio,
		dtype=tf.int32
	)
	max_image_size = tf.cast(
		tf.cast(tf.stack([image_height, image_width]), dtype=tf.float32) * max_padded_size_ratio,
		dtype=tf.int32
	)
  ...
  """
	...
	with tf.name_scope('RandomCropToAspectRatio', values=[image]):
		...
		orig_aspect_ratio = tf.cast(orig_width,
			dtype=tf.float32) / tf.cast(orig_height, dtype=tf.float32)
		new_aspect_ratio = tf.constant(aspect_ratio, dtype=tf.float32)

		def target_height_fn():
			return tf.cast(
				tf.round(tf.cast(orig_width, dtype=tf.float32) / new_aspect_ratio), dtype=tf.int32
			)
		...
		def target_width_fn():
			return tf.cast(
				tf.round(tf.cast(orig_height, dtype=tf.float32) * new_aspect_ratio), dtype=tf.int32
			)
...
def random_pad_to_aspect_ratio(
  ...
  """
	...
	with tf.name_scope('RandomPadToAspectRatio', values=[image]):
		...
		generator_func = functools.partial(tf.random_uniform, [], min_scale, max_scale, seed=seed)
		...
		im_box = tf.stack([0.0, 0.0, target_height / image_height, target_width / image_width])
		boxlist = box_list.BoxList(boxes)
...
def random_black_patches(
  ...
  """
    ...
    """
		...
		generator_func = functools.partial(
			tf.random_uniform, [], minval=0.0, maxval=(1.0 - size_to_image_ratio), seed=random_seed
		)
  ...
  """
	...
	with tf.name_scope('RandomDownscaleToTargetPixels', values=[image]):
		...
		scale_factor = tf.sqrt(
			tf.cast(target_pixels, dtype=tf.float32) / tf.cast(image_pixels, dtype=tf.float32)
		)
		...
		if masks is not None:
			...
			return tf.cond(
				should_apply_resize, resize_fn, lambda: (tf.cast(image, dtype=tf.float32), masks)
			)
		else:
			resize_fn = lambda: _resize_image_to_target(target_height, target_width)
			return tf.cond(
				should_apply_resize, resize_fn, lambda: (tf.cast(image, dtype=tf.float32), )
			)
  ...
  """
	...
	def _apply_patch_gaussian(image):
		...
		y = get_or_create_rand_vars_fn(
			functools.partial(
			tf.random_uniform, [], minval=0, maxval=image_shape[0], dtype=tf.int32, seed=seed
			),
			key='y'
		)
		x = get_or_create_rand_vars_fn(
			functools.partial(
			tf.random_uniform, [], minval=0, maxval=image_shape[1], dtype=tf.int32, seed=seed
			),
			key='x'
		)
		gaussian = get_or_create_rand_vars_fn(
			functools.partial(
			tf.random.normal, image_shape, stddev=gaussian_stddev, dtype=tf.float32, seed=seed
			),
			key='gaussian'
		)
...
def random_resize_method(image, target_size, preprocess_vars_cache=None):
  ...
  """
	...
	resized_image = _apply_with_random_selector(
		image,
		lambda x,
		method: tf.image.resize_images(x, target_size, method),
		num_cases=4,
		preprocess_vars_cache=preprocess_vars_cache,
	...
	with tf.name_scope('ResizeToRange', values=[image, min_dimension]):
		...
		if pad_to_max_dimension:
			...
			new_image = tf.stack(
				[
				tf.pad(
				channels[i], [[0, max_dimension - new_size[0]], [0, max_dimension - new_size[1]]],
...
def resize_to_min_dimension(
  ...
  """
	...
	with tf.name_scope('ResizeGivenMinDimension', values=[image, min_dimension]):
		...
		target_ratio = tf.cast(min_target_dimension,
			dtype=tf.float32) / tf.cast(min_image_dimension, dtype=tf.float32)
		target_height = tf.cast(
		...
		if masks is not None:
			masks = tf.image.resize_nearest_neighbor(
				tf.expand_dims(masks, axis=3), size=[target_height, target_width], align_corners=True
			)
...
def resize_to_max_dimension(
  ...
  """
	...
	with tf.name_scope('ResizeGivenMaxDimension', values=[image, max_dimension]):
		...
		target_ratio = tf.cast(max_target_dimension,
			dtype=tf.float32) / tf.cast(max_image_dimension, dtype=tf.float32)
		target_height = tf.cast(
			tf.cast(image_height, dtype=tf.float32) * target_ratio, dtype=tf.int32
		)
		target_width = tf.cast(
...
def resize_image(
  ...
  Returns:
    ...
    resized_image_shape: A 1D tensor of shape [3] containing the shape of the
      resized image.
  """
	with tf.name_scope('ResizeImage', values=[image, new_height, new_width, method, align_corners]):
		new_image = tf.image.resize_images(
			image, tf.stack([new_height, new_width]), method=method, align_corners=align_corners
		)
...
def random_self_concat_image(
  ...
  """
	...
	# fixed seeds.
	concat_horizontal = (
		tf.random_uniform([], seed=seed + 1 if seed else None) < concat_horizontal_probability
	)

	gen_func = lambda: (concat_vertical, concat_horizontal)
	params = _get_or_create_preprocess_rand_vars(
		gen_func, preprocessor_cache.PreprocessorCache.SELF_CONCAT_IMAGE, preprocess_vars_cache
	)
	...
	def _concat_image(image, boxes, labels, label_weights, axis):
		...
		return [output_images, all_boxes, tf.tile(labels, [2]), tf.tile(label_weights, [2])]
	...
	if label_confidences is not None:
		label_confidences = tf.cond(
			concat_vertical, lambda: tf.tile(label_confidences, [2]), lambda: label_confidences
		)
		outputs.append(
			tf.cond(
			concat_horizontal, lambda: tf.tile(label_confidences, [2]), lambda: label_confidences
			)
		)

	if multiclass_scores is not None:
		multiclass_scores = tf.cond(
			concat_vertical, lambda: tf.tile(multiclass_scores, [2, 1]), lambda: multiclass_scores
		)
		outputs.append(
			tf.cond(
			concat_horizontal, lambda: tf.tile(multiclass_scores, [2, 1]), lambda: multiclass_scores
			)
		)
...
def ssd_random_crop(
  ...
  """
	...
	result = _apply_with_random_selector_tuples(
		tuple(
		t for t in
		(image, boxes, labels, label_weights, label_confidences, multiclass_scores, masks, keypoints)
		if t is not None
		),
...
def ssd_random_crop_pad(
  ...
  """
	...
	return _apply_with_random_selector_tuples(
		tuple(
		t for t in (image, boxes, labels, label_weights, label_confidences, multiclass_scores)
		if t is not None
		),
...
def random_square_crop_by_scale(
  ...
  """
	...
	image_box = [box_ymin / height, box_xmin / width, box_ymax / height, box_xmax / width]
	boxlist = box_list.BoxList(boxes)
	boxlist = box_list_ops.change_coordinate_frame(boxlist, image_box)
	boxlist, indices = box_list_ops.prune_completely_outside_window(boxlist, [0.0, 0.0, 1.0, 1.0])
	boxlist = box_list_ops.clip_to_window(
		boxlist, [0.0, 0.0, 1.0, 1.0], filter_nonoverlapping=False
	)

	return_values = [
		output_image, boxlist.get(), tf.gather(labels, indices), tf.gather(label_weights, indices)
	]

	if masks is not None:
		new_masks = tf.expand_dims(masks, -1)
		new_masks = new_masks[:, ymin:ymax, xmin:xmax]
		new_masks = tf.image.pad_to_bounding_box(new_masks, oy_i, ox_i, image_size, image_size)
		new_masks = tf.squeeze(new_masks, [-1])
...
def get_default_func_arg_map(
  ...
  """
	...
	prep_func_arg_map = {
		...
		random_image_scale: (fields.InputDataFields.image, groundtruth_instance_masks,
							),
		...
		fields.InputDataFields.image, groundtruth_instance_masks,
		),
		random_patch_gaussian: (fields.InputDataFields.image, ),
		autoaugment_image: (fields.InputDataFields.image, fields.InputDataFields.groundtruth_boxes,
						),
		...
		resize_to_range: (fields.InputDataFields.image, groundtruth_instance_masks,
						),
		...
		resize_image: (fields.InputDataFields.image, groundtruth_instance_masks,
					),
