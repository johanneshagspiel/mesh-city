...
class OverlayCreator:
	...
	def create_image_with_more_trees(self, trees_to_add, detection_info, building_instructions):
		"""
		Method to create a gif image that shows how more trees are popping up
		:param trees_to_add: how many trees to add in percentage
		:param detection_info: the file containing where all the trees currently are on the map
		:param building_instructions: the building_instructions of the request
		:return: nothing (a gif image is stored and shown on the main screen)
		"""
		temp_path = Path.joinpath(
		...
		for tree in range(0, trees_to_add):
			tree_to_duplicate = detection_info.information["Objects"][random.randint(
				1, detection_info.information["Amount"]
			)]
			location_to_place_it_to = detection_info.information["Objects"][random.randint(
				1, detection_info.information["Amount"]
			)]
			image_to_paste = temp_image.crop(
				box=(
				float(tree_to_duplicate["xmin"]),
		...
		temp_path_directory = Path.joinpath(
			self.application.file_handler.folder_overview["active_request_path"], "more_trees"
		)
		number_generated = 1
		...
		if "More Trees" not in building_instructions.instructions["Generated"]:
			...
			number_generated = len(
				building_instructions.instructions["Generated"]["More Trees"][1]
			) + 1
	...
	def calculate_new_location(self, what_to_place, where_to_place):
		...
		new_entry = {
			"label": what_to_place["label"],
			"xmin": new_xmin,
			"ymin": new_ymin,
			"xmax": new_xmax,
			"ymax": new_ymax,
			"score": what_to_place["score"],
