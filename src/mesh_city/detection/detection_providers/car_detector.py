...
from mesh_city.util.object_detection.utils import ops as utils_ops

from mesh_city.util.object_detection.utils import label_map_util

from mesh_city.util.object_detection.utils import visualization_utils as vis_util
...
class CarDetector:
	...
	pure_path_to_frozen_graph =  Path.joinpath(
		Path(__file__).parents[2], "resources", "neural_networks", str(MODEL_NAME) ,"frozen_inference_graph.pb"
	)
	...
	TEST_IMAGE_PATHS = [os.path.join(str(Path.joinpath(
		Path(__file__).parents[0], "test_images"
	)), 'image{}.png'.format(i)) for i in
	                    range(1, 3)]
