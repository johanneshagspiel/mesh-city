...
class mapboxproviderTest(unittest.TestCase):
	def setUp(self):
		...
		self.mapbox = MapboxProvider(self.user_info_init,self.quota_manager_init)
	...
	def test_get_and_store(self):
		...
		mapbox.get_and_store_location(latitude=self.latitude, longitude=self.longitude
		                             , zoom=self.zoom, filename="test_google_top_down.png"
		                             , new_folder_path=self.file_path
		                             , response=self.mock_response())
		with open(self.file_path) as image_one:
			recieved_image = image_one.read()
		with open(mock_path) as image_two:
			mock_image = image_two.read()
		self.assertEqual(recieved_image,mock_image)
