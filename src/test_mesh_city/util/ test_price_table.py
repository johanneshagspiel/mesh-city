...
class PriceTableTest(unittest.TestCase):
	...
	def test_calculate_action_price_mapbox(
		self, api_type, action_type, requests, geo_usage, static_usage, price
	):
		json = {
			"type": api_type,
			"api_key": "test",
			"usage":
			{
			"static_map": static_usage, "geocoding": geo_usage, "total": static_usage + geo_usage
			},
			"quota": 1000,
			"date_reset": "2090-05-31",
		}
		image_provider_entity = ImageProviderEntity(file_handler=self.file_handler, json=json)
		price_util = PriceTableUtil(
			image_provider_entity=image_provider_entity, action=[(action_type, requests)]
		)
